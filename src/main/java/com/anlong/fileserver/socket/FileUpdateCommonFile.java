	...
	public void execute(IoSession session, DataInputStream dis,
		...
		try {
			...
			if (fileSendInfo.getSize() > UploadUtils.getFileByteSizeLimit()) {
				logger.info("socket上传文件大小超过最大"
						+ UploadUtils.getFileByteSizeLimit() + "byte限制");
				return;
			}

			/** 校验mongo索引 */
			if (MongoFileIndex.isExistFile(fileSendInfo.getMd5())) {
				String ioPath = DownloadUtils.getFileIOPath(fileSendInfo
						.getMd5());
				String relativePath = MongoFileIndex.getFilePath(fileSendInfo
						.getMd5());
				/** 校验物理磁盘中是否存在 */
				if (new File(ioPath).exists()) {
					...
					logger.info("index exists but file not exists:"
							+ fileSendInfo.getMd5() + "|" + ioPath);
					// out.print("2");
					dos.writeBytes("2");
				}
